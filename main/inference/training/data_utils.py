...
class TextAudioLoaderMultiNSFsid_RMS(tdata.Dataset):
    def __init__(self, hparams):
        self.audiopaths_and_text = load_filepaths_and_text(hparams.training_files)
        self.max_wav_value = hparams.max_wav_value
        self.sample_rate = hparams.sample_rate
    ...
    def _filter(self):
        audiopaths_and_text_new, lengths = [], []
        for audiopath, text, pitch, pitchf, energy, dv in self.audiopaths_and_text:
            if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
                audiopaths_and_text_new.append([audiopath, text, pitch, pitchf, energy, dv])
                lengths.append(os.path.getsize(audiopath) // (3 * self.hop_length))
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        return (spec, wav, phone, pitch, pitchf, dv, energy)

    def get_labels(self, phone, pitch, pitchf, energy):
        phone = np.repeat(np.load(phone), 2, axis=0)
        n_num = min(phone.shape[0], 900)

        return torch.FloatTensor(phone[:n_num, :]), torch.LongTensor(np.load(pitch)[:n_num]), torch.FloatTensor(np.load(pitchf)[:n_num]), torch.FloatTensor(np.load(energy)[:n_num])
...
class TextAudioCollateMultiNSFsid_RMS:
    def __init__(self, return_ids=False):
        self.return_ids = return_ids
    ...
    def __call__(self, batch):
        ...
        return (phone_padded, phone_lengths, pitch_padded, pitchf_padded, spec_padded, spec_lengths, wave_padded, wave_lengths, sid, energy_padded)
...
class TextAudioLoader_RMS(tdata.Dataset):
    def __init__(self, hparams):
        self.audiopaths_and_text = load_filepaths_and_text(hparams.training_files)
        self.max_wav_value = hparams.max_wav_value
        self.sample_rate = hparams.sample_rate
    ...
    def _filter(self):
        audiopaths_and_text_new, lengths = [], []
        for audiopath, text, energy, dv in self.audiopaths_and_text:
            if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
                audiopaths_and_text_new.append([audiopath, text, dv, energy])
                lengths.append(os.path.getsize(audiopath) // (3 * self.hop_length))
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        return (spec, wav, phone, dv, energy)

    def get_labels(self, phone, energy):
        phone = np.repeat(np.load(phone), 2, axis=0)
        n_num = min(phone.shape[0], 900)

        return torch.FloatTensor(phone[:n_num, :]), torch.FloatTensor(np.load(energy)[:n_num])
...
class TextAudioCollate_RMS:
    ...
    def __call__(self, batch):
        ...
        return (phone_padded, phone_lengths, spec_padded, spec_lengths, wave_padded, wave_lengths, sid, energy_padded)
